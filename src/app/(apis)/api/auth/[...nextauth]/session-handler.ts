...
export default async function sessionHandler(
  ...
  try {
    if (session?.user) {
      ...
      if (
        forced_operators.includes(user_data?.email) &&
        !session.user.roles.includes("operator")
      ) {
      ...
      let permissions: FinalPermissionsType = {};
      ...
      if (session.user.roles.includes("operator")) {
        ...
        const roles = await roles_collection
          .find({ name: { $in: user_roles }, status: "active" })
          .toArray();
        roles.forEach((role) => {
